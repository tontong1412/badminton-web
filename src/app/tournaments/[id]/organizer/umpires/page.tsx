...
const Umpire = () => {
  ...
  const handleAddManager = async() => {
    ...
    const response = await axios.post(`${SERVICE_ENDPOINT}/tournaments/add-umpire`, {
  ...
  const handleRemoveManager = async(manager: SimplePlayer) => {
    setButtonLoading(true)
    const response = await axios.post(`${SERVICE_ENDPOINT}/tournaments/remove-umpire`, {
      playerID: manager.id,
      tournamentID: tournament.id
    }, { withCredentials:true })
    setButtonLoading(false)
      ...
      {!tournament
        ...
        : <Box sx={{ display: 'flex' }}>
          ...
          <Box sx={{ width: '100%', maxWidth: 400, m:5 }}>
            <Typography variant='h5'>Umpire</Typography>
            <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
              <ListItem
                ...
                <Autocomplete
                  disablePortal
                  autoHighlight
                  options={playerList?.filter((p) => !tournament.umpires.map((m) => m.id).includes(p.id))}
                  sx={{ width: 250 }}
                  renderInput={(params) => <TextField {...params} label="Add new umpire" />}
              ...
              {
                tournament.umpires.map((m) => <Box key={m.id}>
                  <ListItem
                    secondaryAction={

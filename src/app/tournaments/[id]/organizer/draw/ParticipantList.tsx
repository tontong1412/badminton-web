...
import { Box, Button, Card, CardContent, CardHeader, Divider, FormControl, InputLabel, MenuItem, Paper, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from '@mui/material'
...
const ParticipantList = ({ eventID }: ParticipantListProps) => {
  ...
  useEffect(() => {
    if(draw.group?.length === 0 && event){
      const initDisplayArray = event?.teams.map(() => -1)
      setGroupDisplayArray(initDisplayArray)
      return
    }
    if(event && draw.group && draw.group?.length > 0){

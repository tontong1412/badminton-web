...
const AddQueueModal = ({ visible, setVisible, setMatchList }: AddQueueModalProps) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    const teamA: NewMatchTeam = { players: [], scoreSet: 0, score: 0, serving: 0, isServing: true, receiving: 0, scoreDiff: 0  }
    if(player1)
      teamA.players.push(player1)
    if(player2)
      teamA.players.push(player2)

    const teamB: NewMatchTeam = { players:[], scoreSet: 0, score: 0, serving: 0, isServing: true, receiving: 0, scoreDiff: 0 }
    if(player3)
